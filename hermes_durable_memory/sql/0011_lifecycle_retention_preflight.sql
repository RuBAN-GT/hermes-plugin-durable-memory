-- Lifecycle is canonical record metadata, not an untrusted payload convention.
ALTER TABLE durable_memory.record
    ADD COLUMN valid_from timestamptz,
    ADD COLUMN valid_to timestamptz;
UPDATE durable_memory.record SET valid_from = created_at WHERE valid_from IS NULL;
ALTER TABLE durable_memory.record ALTER COLUMN valid_from SET DEFAULT now();
ALTER TABLE durable_memory.record ALTER COLUMN valid_from SET NOT NULL;
ALTER TABLE durable_memory.record
    ADD CONSTRAINT record_validity_range CHECK (valid_to IS NULL OR valid_to > valid_from);
ALTER TABLE durable_memory.record DROP CONSTRAINT record_status_check;
ALTER TABLE durable_memory.record
    ADD CONSTRAINT record_status_check CHECK (status IN ('active', 'expired', 'tombstoned', 'retracted', 'superseded'));
ALTER TABLE durable_memory.change_request
    ADD COLUMN valid_from timestamptz,
    ADD COLUMN valid_to timestamptz,
    ADD CONSTRAINT change_request_validity_range CHECK (valid_to IS NULL OR valid_from IS NULL OR valid_to > valid_from);
CREATE INDEX record_active_validity ON durable_memory.record (valid_to)
    WHERE status = 'active';

-- A bounded retention pass only transitions records; revisions and evidence remain intact.
CREATE OR REPLACE FUNCTION durable_memory.expire_records(max_records integer)
RETURNS integer LANGUAGE plpgsql SECURITY DEFINER SET search_path = durable_memory, pg_temp AS $$
DECLARE affected integer;
BEGIN
    IF max_records < 1 OR max_records > 500 THEN RAISE EXCEPTION 'expiration limit must be between 1 and 500'; END IF;
    WITH due AS (
        SELECT id FROM durable_memory.record
        WHERE status = 'active' AND valid_to IS NOT NULL AND valid_to <= now()
          AND durable_memory.has_capability(namespace_id, 'approve')
        ORDER BY valid_to, id LIMIT max_records FOR UPDATE
    )
    UPDATE durable_memory.record AS record SET status = 'expired'
    FROM due WHERE record.id = due.id;
...
BEGIN
    ...
    IF request_row.operation = 'update' THEN
        UPDATE durable_memory.record SET revision = next_revision, search_text = request_row.search_text, payload = request_row.payload, valid_from = COALESCE(request_row.valid_from, valid_from), valid_to = request_row.valid_to, updated_by_profile_id = actor_id WHERE id = record_row.id;
...
BEGIN
    ...
    IF NEW.status <> 'active' OR NEW.valid_from > now() OR (NEW.valid_to IS NOT NULL AND NEW.valid_to <= now()) THEN
        ...
        UPDATE durable_memory.embedding_job SET status = 'cancelled' WHERE record_id = NEW.id AND status = 'pending'; RETURN NEW;
    END IF;
...
BEGIN
    ...
    SELECT * INTO exact_row FROM durable_memory.record WHERE namespace_id = candidate_row.namespace_id AND record_type = candidate_row.record_type AND identity_key = candidate_row.identity_key AND status = 'active' AND valid_from <= now() AND (valid_to IS NULL OR valid_to > now()) AND record_type <> '__inventory_definition__' ORDER BY id LIMIT 1;
    IF FOUND THEN
    ...
    ELSE
        ...
        WHERE candidate_embedding.candidate_id = candidate_row.id AND candidate_embedding.lifecycle_status = 'indexed' AND record.namespace_id = candidate_row.namespace_id AND record.record_type = candidate_row.record_type AND record.record_type <> '__inventory_definition__' AND record.status = 'active' AND record.valid_from <= now() AND (record.valid_to IS NULL OR record.valid_to > now()) AND projection.revision = record.revision AND durable_memory.has_capability(record.namespace_id, 'propose') ORDER BY projection.embedding <=> candidate_embedding.embedding, record.id LIMIT 1;
...
-- Migration ownership needs owner access; preflight rejects any runtime table owner.
REVOKE ALL ON FUNCTION durable_memory.expire_records(integer) FROM PUBLIC;
